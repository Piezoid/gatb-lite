...
bool
is_minimizer_allowed(uint32_t mmer)
{
    // TODO
...
revcomp(const uint64_t& x, int k)
{
    uint64_t res = x;
...
void
init_minimizers(int _minimizerSize, vector<uint32_t>& _mmer_lut)
{
    ...
    for (uint32_t ii = 0; ii < nbminims_total; ii++) {
        ...
        if (!is_minimizer_allowed(mmer), _minimizerSize) mmer = _mask;
...
void
compute_minimizer(uint64_t                kmer,
                  uint32_t&               minimizer,
                  unsigned int&           minimizer_position,
                  unsigned int            k,
                  const vector<uint32_t>& _mmer_lut,
                  unsigned int            _minimizerSize)
{
    minimizer = 0;
    uint32_t mmer;
...
void
read_to_superkmer(const char*             read_start,
                  const char*             read_end,
                  const vector<uint32_t>& _mmer_lut,
                  unsigned int            k,
                  unsigned int            _minimizerSize,
                  uint64_t&               nb_kmers_thread,
                  uint64_t&               nb_superkmers_thread)
{
    ...
    while (buffer_index < readlen - k + 1) {
        ...
        while (buffer_index < readlen - k + 1) {
            ...
            if (unlikely(read_start[i] == 'N')) {
                buffer_index = i + 1;
                break;
            } // no need to set found_N here, we'll continue
            unsigned char c = char_to_nt(read_start[i]);
...
main(int argc, char** argv)
{
    ...
    vector<vector<const char*>> read_positions(nb_threads); // fill a [for now non-circular] buffer of read position,
                                                            // written by the IO thread and read by the worker threads
    ...
    const uint64_t      read_positions_buffer_size
      = (1
         << 26); // process 70M reads/thread at a time (memory usage of 140MB/thread to remember parsed read positions)
    ...
    for (int thread_id = 0; thread_id < nb_threads; thread_id++) {
        ...
        auto thread_func = [&thread_over,
                            &thread_caught_up,
                            read_positions_buffer_size,
                            nb_threads,
                            thread_id,
                            &read_positions,
                            &nb_read_positions,
                            &nb_kmers_threads,
                            &nb_superkmers_threads,
                            &_mmer_lut,
                            &k,
                            &_minimizerSize]() {
            ...
            while ((!thread_over[thread_id]) || (position_index < nb_read_positions[thread_id])) {
                while ((position_index >= nb_read_positions[thread_id]) && (!thread_over[thread_id])) {
                    if (thread_caught_up[thread_id]
                        == false) // signal that we're going to reset read_positions and nb_read_positions
                    {
                        if (!(position_index
                              >= nb_read_positions[thread_id])) // needed to avoid false alerts (which result in
                                                                // slightly less sequences processed: to reproduce,
                                                                // uncomment that "if" statement and set
                                                                // read_positions_buffer_size to 1<<11), and make sure
                                                                // to sleep(0.1) just before the parent "if" (not after)
                            break;
                    ...
                    sleep(
                      0.1); // FIXME: code loops forever if that sleep() is missing; would love to figure out why; to
                            // reproduce: comment that line and do benchs/read_superkmers benchs/frag_1.fastq.20klines 3
                    continue;
                }                                                   // waiting on IO, shouldn't happen so often
                if (position_index >= nb_read_positions[thread_id]) // avoid case when thread is over but position_index
                                                                    // is already too high
                    continue;
                ...
                position_index += 2;
                read_to_superkmer(
                  read_start, read_end, _mmer_lut, k, _minimizerSize, nb_kmers_thread, nb_superkmers_thread);
            }
    ...
    process_fastq(
      argv[1], [nb_threads, &nb_reads, &nb_read_positions, &read_positions, &thread_caught_up](fastq_record<>& rec) {
          ...
          if (unlikely((nb_read_positions[thread_id] > 0)
                       && (nb_read_positions[thread_id] % read_positions_buffer_size == 0))) {
    ...
    for (int i = 0; i < nb_threads; i++) {
        threads[i].join();
        nb_kmers += nb_kmers_threads[i];
        nb_superkmers += nb_superkmers_threads[i];
    }
